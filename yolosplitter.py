...
class YoloSplitter():
    ...
    """
    def __init__(self,imgFormat= [".jpg",".JPG",".JPEG", ".jpeg", ".png",".PNG"],labelFormat=[".txt"]):
    ...
    def from_yolo_dir(self,input_dir,ratio=(0.70,0.20,0.10),return_df=False):
        ...
        set_dir_names = [i  for i in  os.listdir(input_dir) if i in  ["train","test","val"]]
        all_dataframes = []
    ...
    def __make_split(self,input_df,ratio):
        ...
        set_names = ['train'] * train_length + ['val'] * val_length + ['test'] * test_length
    ...
    def save_split(self,output_dir):
        ...
        if os.path.exists(output_dir):
            print("Output directory {} already exists, rewriting content inside")
        ...
        train_set,valid_set,test_set = [self.__DATAFRAME.new_set.value_counts().to_dict()[i] if i in self.__DATAFRAME.new_set.value_counts().to_dict()  else 0 for i in ["train","val","test"]]

        yamlFile["nc"] = len(cls_names)
        yamlFile["names"] = cls_names
        yamlFile["train"] = os.path.join(output_dir, "train")
        os.makedirs(yamlFile['train'],exist_ok=True)
        if valid_set != 0:
            yamlFile["val"] = os.path.join(output_dir, "val")
            os.makedirs(yamlFile["val"],exist_ok=True)
        if test_set != 0:
            yamlFile["test"] = os.path.join(output_dir, "test")
            os.makedirs(yamlFile["test"],exist_ok=True)

        with open(os.path.join(output_dir, "data.yaml"), "w") as f:
            yaml.dump(yamlFile, f, indent=2,default_flow_style=None)

        train_dir = yamlFile["train"]
        test_dir = yamlFile["test"]
        val_dir = yamlFile["val"]

        for directory in train_dir,test_dir,val_dir:
            for image_file,label_file in zip(os.listdir(os.path.join(directory,'images')),os.listdir(os.path.join(directory,'labels'))):
                image_src = os.path.join(directory,'images',image_file)
                label_src = os.path.join(directory,'labels',label_file)
                image_dest = directory
                label_dest = directory
                try:
            ...
            os.rmdir(os.path.join(directory,'images'))
            os.rmdir(os.path.join(directory,'labels'))